...
unsigned char subByte(unsigned char c) {
	unsigned char x = c >> 4;
	unsigned char y = c - (x << 4);
	return sBoxEn[x][y];
}
...
unsigned char** keyExpansion(string &key) {
	int wordsNo = 4 * (roundsNo + 1),
		i = 0;
	unsigned char **word = new unsigned char*[wordsNo];
	while (i < keyColumnsNo) {
		word[i] = new unsigned char[4];
	...
	while (i < wordsNo) {
		...
		if (i%keyColumnsNo == 0) {
			rotateLeft(temp);
			subWord(temp);
			temp = xor(temp, rCon[i / keyColumnsNo]);
		}
		else if (keyColumnsNo > 6 && i%keyColumnsNo == 4) subWord(temp);
		word[i] = xor(word[i - keyColumnsNo], temp);
		i++;
	}
	return word;
}

unsigned char** getRoundKey(unsigned char **word, int round) {
	unsigned char **roundKey = new unsigned char*[4];
	for (int i = 0; i < 4; i++) roundKey[i] = new unsigned char[4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) 
...
void addRoundKey(unsigned char **state, unsigned char **roundKey) {
	for (int i = 0; i < 4; i++) state[i] = xor (state[i], roundKey[i]);
}
...
string aesEn(string &input, unsigned char **word) {
	unsigned char **state = intoState(input);
	unsigned char **roundKey;
	roundKey = getRoundKey(word, 0);
	addRoundKey(state, roundKey);
	for (int roundNo = 1; roundNo < roundsNo; roundNo++) {
		roundKey = getRoundKey(word, roundNo);
		round(state, roundKey);
	}
	subBytes(state);
	shiftRows(state);
	roundKey = getRoundKey(word, roundsNo);
	addRoundKey(state, roundKey);
	return intoString(state);
}
...
unsigned char invSubByte(unsigned char c) {
	unsigned char x = c >> 4;
	unsigned char y = c - (x << 4);
	return sBoxDe[x][y];
}

void invSubWord(unsigned char *word) {
	for (int i = 0; i < 4; i++) {
...
string aesDe(string &input, unsigned char **word) {
	...
	invSubBytes(state);
	for (int roundNo = 1; roundNo < roundsNo; roundNo++) {
		roundKey = getRoundKey(word, roundsNo - roundNo);
		roundDe(state, roundKey);
	}
	roundKey = getRoundKey(word, 0);
	addRoundKey(state, roundKey);
	return intoString(state);
